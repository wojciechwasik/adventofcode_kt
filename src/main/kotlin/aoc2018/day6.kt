...
fun main(args: Array<String>) {
    ...
    // normalize
    val minX = sources.minBy { it.x } ?.x ?: abort("This should never happen")
    val minY = sources.minBy { it.y } ?.y ?: abort("This should never happen")
    val normalized = sources.map { it.transform(-minX, -minY) }

    val maxX = sources.maxBy { it.x } ?.x ?: abort("This should never happen")
    val maxY = sources.maxBy { it.y } ?.y ?: abort("This should never happen")
